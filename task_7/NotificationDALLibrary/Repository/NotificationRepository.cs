...
namespace NotificationDALLibrary.Repository
{
    public class NotificationRepository : IRepository<int, Notification>
    {
        ...
        private int LastMessageid = 0; // variable for storing unique message id
        ...
        {
            var notification = _notificationMap.FirstOrDefault(n => n.Key == key).Value; //checking the message is present using LINQ
        ...
        {
            //using LINQ for getting the notification details
            return _notificationMap.FirstOrDefault(n => n.Key == key).Value;
        }
        ...
        {
            var notification = _notificationMap.FirstOrDefault(n => n.Key == key).Value; //calling the method to check the notification is present
            if (notification != null)
            {
